...
在这个工作中，我们重点在流上的多维时间序列，它是空间依赖是高度非线性的，并且数据是在线分析的。Yamanishi et al.提出了，提出了时间序列在流中的SmartSifter algorithm，该算法采用 online discounting learning algorithm来逐步学习概率混合模型，用衰减因子来解释异常。除了online discounting learning algorithm之外，大量方法使用dynamically maintained cluster models来计算数据流中的异常值，例如论文`ADMIT: Anomaly-based data mining for intrusions`。

我们也在异常检测中加入了图结构的数据。目前现有的工作只学习了时间间隔h的graph behavior，对长、复杂和重复的patterns效果并不好。

于是我们结合variation inference(VI)和RNN对图上的时间序列建模，并且用log likelihood检测异常。时间序列模型是基于VRNN的(variational RNN)。而在空间数据上，用的GCNN处理图数据。

### Conclusion

本文展示了`variational inference and recurrent neural networks`在时间序列异常检测的应用。在对比试验中证明了模型在图的时间序列上的异常检测中效果不错，`spectral lter`可以更好的学习空间信息，`variational inference and recurrent neural networks`对时间相关性进行建模。`multi-variable Gaussian model `对异常值足够敏感。与之前工作不同的是，我们的模型不仅可以检测异常时间点，还可以定位图上的异常节点。未来计划将该模型用于视频异常检测。
...
Learning on graphs有几类方法

- random walks
- counting subgraphs
- spectral ideas
- label propagation schemes with hashing
- algebraic ideas
...
Steerability意味着在给定的神经元上的activations(特征向量)必须根须视野域的特定表达来转换。在本文中我们只考虑定义其表示形式和其张量积。
...
评估图之间的相似性很重要，在本文中，我们提出了一种新的度量学习方法评估图之间的距离，同时借助谱图理论将此方法应用在不规则图上。在这个问题中，适当的衡量图的相似性的函数的定义对揭示与某些大脑疾病相关的干扰模式至关重要。在Abide数据集上的实验结果表明，我们的方法可以学习针对临床应用量身定制的图的相似性度量，与传统的距离度量相比，简单分类器的性能提高了11.9％。据我们所知，这个GCN在度量学习上的首次应用。

- 将siamese graph convolutional neural network应用与不规则图，并使用了polynomial filters
- 应用了全局loss function
- 能够学习更好的关于图的潜在表达
...
在point clouds上的特征学习引入了如point-net++的深度学习框架，很有前景。但是到目前为止，仅利用了point的特征，忽略了与邻接点的连接及邻接点的特征。在本文中，我们用spectral graph convolution和新的图池化策略克服了这个困难。我们用递归聚类和池化策略取代了max pooling，该策略在spectral coordinates中判断节点相似性，并且聚类，然后汇总clusters里面的节点的信息，从而得到更好的特征表达。
...
最近GCN得到了机器学习社区广泛的关注。他们探索了graph Laplacian eigenvectors和传统Fourier basis之间的类比，从而将卷积公式表示为spectral domain中的乘法。spectral CNNs的一个关键的缺点是他们对无向图明确的假设，导致导致具有正交特征分解(orthogonal eigendecom-position)的对称拉普拉斯矩阵。在本文中，我们提出了MotifNet，一个能利用local graph motifs处理有向图的graph CNN。我们使用了一种注意力机制，使得MotifNet一般化标准的graph CNN，并且不会显著地增加模型复杂度。
...
为了解决这些问题，我们提出了hierarchical feature aggregation approach，它会在不同深度的邻域上学习不同的特征聚合器。然后使用LSTM聚合不同深度的信息，从而确保信息从更高的深度流向节点。
...
- Graph-augmented neural network可以用于许多神经网络；另外，这种技术可以用于inductive and transductive setting；有助于其他神经网络在label很少的数据中训练
